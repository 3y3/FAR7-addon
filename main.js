	...
	this.trade = new function tradeProcess(){
		...
		this.interface = new function(){
			...
			var tradeSrc = 
				...
				+'</span></span></a></span>';
			commbarContainer[HT] += tradeSrc;
			
			var tB = S['TradeButton'] = commbarContainer[QS]('#addon-trade-button');
				tB[AE]('click', dd);
			...
			var tDsrc =
				...
				+'</div>';
			far[HT] += tDsrc;
			var tD = S['TradeDialog'] = far[QS]('#addon-trade-panel');
			...
			tD[QS]('.jxFlyoutContent')[HT] += tDCsrc;
			var tDC = S['TradeDialogContent'] = tD[QS]('#landing-trade');
				tDC.parentNode[RC](tDC); /*TODO: Try Event mousedown & not removing*/
			var DT = S['data'] = {};
			...
			function ui(){			
				for(var i in DT){
					...
					(obj.buy = buy[0]) 	 || (DT.buy.dom[HT] = '-');
					(obj.sell = sell[0]) || (DT.sell.dom[HT] = '-');
