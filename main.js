	...
	this.trade = new function tradeProcess(){
		...
		this.interface = new function(){
			...
			this.tradeButton = DOMP.parseFromString(tradeSrc,'text/html').documentElement;
				this.tradeButton.addEventListener('click', displayDialog, false);
				commbarContainer[a](this.tradeButton);
				commbar[s].width = '1000px';
			
				var rect = this.tradeButton.getClientRects()[0];
				var dialodSrc =
					'<div class="jxFlyout jxHasChrome" style="left: '+(rect.left - 5)+'px; top: 25px; z-index: 1000;width: 670px; height: 375px;display: none;">'
			...
			function displayDialog(e){
				e.stopPropagation();
				if(tradeDialog[s].display == 'none'){
					updateInfo();
					interfase.tradeDialog.children[0][a](tradeDialogContent);
					tradeDialog[s].display = 'block';
					window.addEventListener('click', hideDialod, false);
					interfase.tradeButton.removeEventListener('click',displayDialog);
				}
			}
			function hideDialod(e){
				e.stopPropagation();
				if(tradeDialog[s].display == 'block'){
					tradeDialog[s].display = 'none';
					interfase.tradeDialog.children[0].removeChild(tradeDialogContent);
					window.removeEventListener('click', hideDialod);
					interfase.tradeButton.addEventListener('click', displayDialog, false);
				}
				return false;
			}
			function updateInfo(){
			
				for(var i in interfase.data){
					var cur_c = tradeDialogContent[q]('#s'+i);
					if(!cur_c[q]('#is'+i)){cur_c[a](interfase.data[i].img.dom)}
					if(!cur_c[q]('#is'+i+'c')){cur_c[a](interfase.data[i].count.dom)}
					if(!cur_c[q]('#is'+i+'sell')){cur_c[a](interfase.data[i].buy.dom)}
					if(!cur_c[q]('#is'+i+'buy')){cur_c[a](interfase.data[i].sell.dom)}					
					if(!cur_c[q]('#is'+i+'arrow')){cur_c[a](interfase.data[i].arrow.dom)}
					var buy = trade.buy(i);
					var sell = trade.sell(i);
					if(buy[0]){
						buy = buy[0];
						interfase.data[i].buy.dom.innerHTML = buy.buy+'кр.';
						interfase.data[i].buy.dom.setAttribute('title','Система: '+buy.system+'\nПланета: '+buy.planet+'\nОбновлено в '+buy.time.toLocaleTimeString());
						interfase.data[i].buy.info.system = buy.system;
						interfase.data[i].buy.info.planet = buy.planet;
						interfase.data[i].buy.info.lastupdate = buy.time;
					}
					else{
						interfase.data[i].buy.dom.innerHTML = '-';
					}
					if(sell[0]){
						sell=sell[0];
						interfase.data[i].sell.dom.innerHTML = sell.sell+'кр.';
						interfase.data[i].sell.dom.setAttribute('title','Система: '+sell.system+'\nПланета: '+sell.planet)
						interfase.data[i].sell.info.system = sell.system;
						interfase.data[i].sell.info.planet = sell.planet;
						interfase.data[i].sell.info.lastupdate = sell.time;
					}
					else{
						interfase.data[i].sell.dom.innerHTML = '-';
					}				
				}
